...
These notes are available in .pdf form at the following link: \url{https://github.com/k-dineen/MAT367-Course-Notes}

The following are course notes for the course MAT367 (Differential Geometry) offered in the Summer of 2020, taught by Ahmed Ellithy. The course notes are mostly based off of handwritten notes created during lectures. There are like many typos as the author has not taken the time to revise the notes. If you find any typos, please try to contact the author at the above link.

The chapters (Manifolds, Smooth Maps and Submanifolds, The Tangent Bundle and Lie Derivative, Differential Forms, and finally Integration on Manifolds) are not meant to be specific. Some content that should be in one chapter often spills over to the next, and so on.

The exposition on topics given here is not always that of the instructor. The author has chosen to insert his own exposition on topics here and there, and to give proofs of things that were left as exercises. These proofs or expositions may be incorrect.
...
\chapter{Smooth Maps and Submanifolds}
...
\chapter{The Tangent Bundle and Lie Derivative}
...
\subsection{The Tangent Space at I (Unfinished)}
\subsection{Left-Invariant Vector Fields (Unfinished)}
\subsection{Lie Algebras (Unfinished)}
\subsection{The Differential as a Lie Algebra Homomorphism (Unfinished)}
...
\[
i_v(\alpha^1 \wedge \cdots \wedge \alpha^k) = \sum_{i=1}^k (-1)^{i-1} \alpha^i(v) \alpha_1 \wedge \cdots \wedge \widehat{\alpha^i} \wedge \cdots \wedge \alpha^k.
\]
...
The following corollary of this characterization of orientability provides a wealth of oriented manifolds for us to work with. The proof was left as an exercise in class. The proof we will give provides a technique for finding \emph{orientation forms} for such regular hypersurfaces, and once Riemannian manifolds are developed, the result may be generalized. (That said, the proof already shows something a lot more general than the statement. An \emph{orientation form} is a non-vanishing smooth top degree form giving the orientation on $M$ as in the preceding theorem.)

\begin{corollary}
Any regular hypersurface in $\R^n$ is orientable. 
\end{corollary}
\begin{proof}
Suppose $S = f^{-1}(\{0\})$, where $f : \R^n \to \R$ is a smooth function with $0$ as a regular value. Denote by $\nabla f$ the gradient vector field of $f$ on $\R^n$. This vector field is, by assumption, non-zero on $S$. Let $\omega$ be an orientation form on $\R^n$ (for example, we can take $\omega = dx^1 \wedge \cdots \wedge dx^n$). Let $\eta = i_S^*(i_{\nabla f}(\omega))$, where $i_{\nabla f}$ is interior multiplication by $\nabla f$ and $i_S : S \hookrightarrow M$ is the inclusion map. Then $\eta$ is a smooth top degree form on $S$. Given $p \in S$, $\left. \nabla f \right|_p \in ((i_S)_{*,p}(T_pS))^\perp$ is non-zero, so if $\{v_1, \dots, v_{n-1}\}$ is a basis of $T_pS$, we have
\[
\eta_p(v_1, \dots, v_{n-1}) = \omega_p(\left. \nabla f \right|_p, (i_S)_{*,p}(v_1), \dots, (i_S)_{*,p}(v_{n-1})) \neq 0,
\]
for $\{ \left. \nabla f \right|_p, (i_S)_{*,p}(v_1), \dots, (i_S)_{*,p}(v_{n-1}) \}$ is a basis of $T_p\R^n$ and $\omega_p \neq 0$. Then $\eta$ is a non-vanishing smooth top degree form on $S$, so $S$ is orientable by the previous theorem.
\end{proof}
...
\section{Integration on Manifolds (August 13)}

This is the final lecture. We will motivate and develop the theory of integration on manifolds. We will state Stokes' theorem, outline a proof, and attempt to provide some very rough intuition for the formula.

\subsection{Why Integrate Forms?}

Due to the change-of-variables formula for integration, we cannot give a well-defined notion of the integral of a function on a smooth manifold. We can integrate functions on $\R^n$ because of the standard set of coordinates. Precisely, there is a natural isomorphism $\Omega^0(\R^n) \to \Omega^n(\R^n)$ given by $f \mapsto f \, dx^1 \wedge \cdots \wedge dx^n$. On an arbitrary smooth manifold we do not have the luxury of such an isomorphism, and so we cannot give a good definition of the integral of a function that is intrinsic to the manifold. 

However, differential forms possess the properties desired for integration. In order to see what this means, we will first take a look at alternating tensors on vector spaces.

Let $V$ be a vector space and let $\alpha \in V^*$ be a covector. Suppose $v \in V$ is non-zero. As we have given no inner product structure to $V$, we cannot speak of the length of $v$. However, the linearity of the covector $\alpha$ means that it has the properties that we would expect a "signed length meter" to have. Namely, if we scale the vector $v$ by some amount $c$, then $\alpha$ respects that scaling, in that $\alpha(cv) = c \alpha(v)$. We can think of $\alpha$ as a kind of "ruler" on $V$. (This begs the question: "Why don't we define a "length" on $V$ in terms of $\omega$?" One answer is that it would not be the same as the usual notion of length if we introduce an inner product.)

Now suppose $\omega \in \Omega^1(M)$. Let $\gamma : (a, b) \to M$ be an embedding, so that $S \coloneqq \gamma((a, b))$ is an embedded $1$-dimensional submanifold of $M$. For each $t \in (a, b)$, $\omega_{\gamma(t)}$ is a "signed length meter" on $T_{\gamma(t)} S = \mathrm{span}(\{\gamma'(t)\})$. If we think of $\omega_{\gamma(t)}(\gamma'(t))$ as representing some kind of infinitesimal length of $\gamma'(t)$, then we would like to "add them all up" using integration. We therefore define the "signed length" of $S$ by
\[
\ell(S) \coloneqq \int_S \omega = \int_a^b \omega_{\gamma(t)}(\gamma'(t)) \, dt.
\]
This is not intrinsic to the manifold, since it depends on the choice of $\omega$. However, it does not depend on the parametrization of $S$. This is due to the properties of $\omega$ as a differential form. Let $\psi : (c,d) \to (a,b)$ be an increasing diffeomorphism. We have 
\begin{align*}
\int_{(c, d)} \omega_{(\gamma \circ \psi)(t)}((\gamma \circ \psi)'(t)) \, dt &= \int_{\psi^{-1}((a,b))} \omega_{(\gamma \circ \psi)(t)}((\gamma \circ \psi)'(t)) \, dt \\
&= \int_{(a,b)} \omega_{\gamma(t)}((\gamma \circ \psi)'(\psi^{-1}(t))) \cdot |(\psi^{-1})'(t)| \, dt \\
&= \int_{(a,b)} \omega_{\gamma(t)}(\gamma'(t)) \cdot \left( \psi'(\psi^{-1}(t)) \cdot (\psi^{-1})'(t) \right) \, dt \\ 
&= \int_{(a,b)} \omega_{\gamma(t)}(\gamma'(t)) \cdot \left( \psi'(\psi^{-1}(t)) \cdot \frac{1}{\psi'(\psi^{-1}(t))} \right) \, dt \\
&= \int_{(a,b)} \omega_{\gamma(t)}(\gamma'(t)) \, dt,
\end{align*}
where in the second equality we used the change of variables theorem for integration, in the third we used the fact that $\psi$ is increasing as well as the properties of $\omega_{\gamma(t)}$ as an alternating tensor, and in the fourth we used the inverse function theorem of first year calculus. This example illustrates how differential forms have the properties desired for integration; \emph{they respect changes of variables.}

Returning to our vector space example, let $W \subseteq V$ be a $k$-dimensional subspace of $V$. As we should now expect, a "signed $k$-dimensional volume meter" on $W$ will be an alternating $k$-tensor. Consider for simplicity the case $k = 2$. Fix two non-zero vectors $v_1,v_2 \in W$ and consider the parallelepiped spanned by them. When we try to measure the $2$-dimensional volume of this parallelepiped, we would expect that when we scale any vector by some amount, the volume scales respectively. Moreover, we would expect that if we swap $v_1$ and $v_2$, the volume changes sign; this is precisely the alternating property! Thus the elements of $\bigwedge^k(V^*)$ have precisely the properties desired of "$k$-dimensional signed volume meters." 

\subsection{Integrating Forms, Simple Case}

Fix $\omega \in \Omega^k(M)$ and a $k$-dimensional oriented submanifold $i : S \hookrightarrow M$.

For each $p \in S$, $\omega_p$ is a "signed $k$-dimensional volume meter" on $T_pS = i_{*,p}(T_pS)$. (We will proceed to make this abuse of notation from now on.) Choose linearly independent tangent vectors $X_1|_p, \dots, X_k|_p \in T_pS$ which are consistent with the orientation on $T_pS$, and do this for each $p \in S$. We would like to "add up" all of the $\omega_p(X_1|_p, \dots, X_k|_p)$'s; this is done via integration.

Suppose for simplicity that $S$ is covered by a single oriented chart $\phi = (x^1, \dots, x^k) : S \to \R^k$. We attempt to define
\[
\int_S \omega \coloneqq \int_{\phi(S)} \omega\left( \frac{\pd}{\pd x^1}, \dots, \frac{\pd}{\pd x^k} \right) \circ \phi^{-1}.
\]
We require that $\omega$ is compactly supported for this integral to be defined. Does this depend on the coordinate chart? No. The properties of $\omega$ as a differential form ensure that this integral is well-defined. (Compare with the case when we integrated a covector field over an embedded curve and showed that the integral did not depend on the parametrization of the curve.) Let $\psi = (y^1, \dots, y^k)$ be another global oriented chart for $S$. We compute
\begin{align*}
\int_{\phi(S)} \omega\left( \frac{\pd}{\pd x^1}, \dots, \frac{\pd}{\pd x^k} \right) \circ \phi^{-1} &= \int_{\phi(S)} \left( \omega\left( \frac{\pd}{\pd y^1}, \dots, \frac{\pd}{\pd y^k} \right) \circ \phi^{-1} \right)\det(D(\psi \circ \phi^{-1})) \\
&= \int_{(\phi \circ \psi^{-1})(\psi(S))} \left( \omega\left( \frac{\pd}{\pd y^1}, \dots, \frac{\pd}{\pd y^k} \right) \circ \phi^{-1} \right)\det(D(\psi \circ \phi^{-1})) \\
&= \int_{\psi(S)} \left( \omega\left( \frac{\pd}{\pd y^1}, \dots, \frac{\pd}{\pd y^k} \right) \circ \psi^{-1} \right) \det(D(\psi \circ \phi^{-1})(\phi \circ \psi^{-1})) \left| \det(D(\phi \circ \psi^{-1})) \right| \\
&= \int_{\psi(S)} \omega\left( \frac{\pd}{\pd y^1}, \dots, \frac{\pd}{\pd y^k} \right) \circ \psi^{-1},
\end{align*}
implying that the integration is independent of the choice of oriented chart. If we had chosen an oppositely-oriented chart, then we would have the negative of the original integral, as we can see going from the third line to the fourth. It is the first step that illustrates the property of differential forms that we desire for a theory of integration on manifolds. (One can check that the integration we just defined is equivalent to the definition of integration developed in MAT257.)

\subsection{Integrating Forms In General}

In the previous subsection's example, we assumed that $\omega$ was compactly supported in order for the integral to actually exist. We will make this assumption from now on in order to integrate forms and not worry about existence issues. What happens if the support of $\omega$ is not covered by a single chart? We integrate locally and use a partition of unity to patch things up.

Let $\omega \in \Omega^n(M)$ be a compactly supported form on the orientable manifold $M$. Let $\{ (U_\alpha, \phi_\alpha) \}$ be an oriented atlas for $M$. Each restriction $\omega|_{U_\alpha}$ is compactly supported in $U_\alpha$, and $U_\alpha$ is an $n$-dimensional submanifold of $M$ covered by a single coordinate chart, so we can use the theory of integration developed in the previous subsection to say that $\int_{U_\alpha} \omega|_{U_\alpha}$ exists. 

Now let $\{\rho_\alpha\}$ be a partition of unity subordinate to $\{U_\alpha\}$. Each form $\rho_\alpha \omega|_{U_\alpha} = \rho_\alpha \omega$ is compactly supported in $U_\alpha$, so the integral
\[
\int_{U_\alpha} \rho_\alpha \omega = \int_{\phi_\alpha(U_\alpha)} (\rho_\alpha \omega)\left( \frac{\pd}{\pd x_\alpha^1}, \dots, \frac{\pd}{\pd x_\alpha^k} \right) \circ \phi_\alpha^{-1} 
\]
that we defined in the previous subsection exists. Now we define
\[
\int_M \omega \coloneqq \sum_\alpha \int_{U_\alpha} \rho_\alpha \omega.
\]
This has the fortunate consequence $\int_M \rho_\alpha \omega = \int_{U_\alpha} \rho_\alpha \omega$. In order for this to be well-defined we must check that it is independent of the choice of partition of unity. (We essentially already saw that it is also independent of the oriented atlas.)

\begin{proposition}
The following properties hold:
\begin{enumerate}
\item
The integral $\int_M \omega$ is independent of the partition of unity used.
\item
The sum is finite. 
\item
$\int_{-M} \omega = -\int_M \omega$, where $-M$ denotes $M$ with the opposite orientation, assuming $M$ is connected.
\item
Integration of compactly supported forms is linear.
\end{enumerate}
\end{proposition}

The proofs are nothing new from what we have seen in MAT257.

\subsection{Back to Euclidean Space}

Recall that on $\R^n$ there is a natural isomorphism $\Phi : \Omega^0(\R^n) \to \Omega^n(\R^n)$ defined by $\Phi(f) \coloneqq f \, dx^1 \wedge \cdots \wedge dx^n$. Does the definition of integration on manifolds we just developed generalize the integration we know of functions on $\R^n$? Yes.

Suppose $f \in \Omega^0(\R^n)$ is compactly supported in some open set $U$. Let $\omega = \Phi(f)$. We have
\begin{align*}
\int_U f(x^1, \dots, x^n) dx^1 \cdots dx^n &= \int_U f(x^1, \dots, x^n) dx^1 \wedge \cdots \wedge dx^n \left( \frac{\pd}{\pd x^1}, \dots, \frac{\pd}{\pd x^n} \right) dx^1 \cdots dx^n \\
&= \int_U \omega\left( \frac{\pd}{\pd x^1}, \dots, \frac{\pd}{\pd x^n} \right) dx^1 \cdots dx^n \\
&= \int_U \omega.
\end{align*}

So we have indeed generalized integration from $\R^n$ to manifolds.

\subsection{A Note On Integrating Functions}

On an abstract smooth manifold there is no such isomorphism intrinsic to the manifold. If $\omega \in \Omega^n(M)$ is an orientation form for $M$, then we might try to define $\Phi_\omega : \Omega^0(M) \to \Omega^n(M)$ by $\Phi_\omega(f) \coloneqq f \omega$. As one can check, $\Phi_\omega$ is an isomorphism; this fact relies on $\omega$ vanishing nowhere. Then we can define the integration of compactly supported functions on $M$ by
\[
\int_M^{(\omega)} f \coloneqq \int_M f \omega
\]
(the notation we have chosen is meant to illustrate the dependence on the orientation form). Since this is dependent on $\omega$, this definition of integration is not intrinsic to the manifold. On abstract smooth manifolds without any extra structure, we cannot define an integration of functions that is intrinsic to the manifold.

When the volume form on an oriented Riemannian manifold is developed, we will be able to, for if $\omega$ is the volume form, our isomorphism is $f \mapsto f \omega$, which is an $n$-form which we will soon see how to integrate. Since $\omega$ depends only on the structure of the Riemannian manifold, we have a definition of the integral of a function on an oriented Riemannian manifold that is intrinsic to the manifold. The definition of a volume form on an oriented Riemannian manifold is almost identical to the definition given in MAT257 for submanifolds of $\R^n$.

\subsection{Stokes' Theorem}

Note that everything we have done so far was for manifolds without boundary. The boundary of a manifold with boundary has measure zero, so it does not contribute to the integration. Thus we can extend everything we have just done to manifolds with boundary, since the contribution to the integral by the boundary will always be zero.

In order to elegantly state Stokes' theorem, we make the following abuse of notation: if $M$ is a manifold with boundary $i : \pd M \hookrightarrow M$, and $\omega \in \Omega^n(M)$, then we write $\int_{\pd M} \omega$ to mean the integral $\int_{\pd M} i^*\omega$.

We now state the final big theorem of the course.

\begin{theorem}
(Stokes') Let $M$ be an oriented manifold with boundary and give $\pd M$ the induced orientation. If $\omega \in \Omega^{n-1}(M)$ is compactly supported, then
\[
\int_M d\omega = \int_{\pd M} \omega.
\]
\end{theorem}

Note that when $n = 0$, this is just the fundamental theorem of calculus. (An orientation on a $0$-manifold is an assignment of a sign $+1$ or $-1$ to each point; the induced orientation on the boundary of $[a, b]$ is $-1$ on $a$ and $+1$ on $b$.)

The hard work required to prove this has been done in MAT257, and so we will not provide a proof. (Sorry!) Instead, we will attempt to provide some motivation for what the formula means and make it more intuitive. The following exposition will be very, \emph{very} rough and informal. This part was not done in the lecture, and is the author's lazy attempt to provide some intuition.

Consider a $2$-dimensional oriented manifold with boundary $M$ in $\R^2$, as pictured. 
\begin{figure}[H]
\centering
\includegraphics[scale=0.3]{stokes1.png}
\end{figure}
Let $\omega \in \Omega^1(M)$. We can think of calculating the integral $\int_M d\omega$ as, very roughly, summing up oriented infinitesimal volumes in $M$, so that their boundaries cancel out and leave only the contribution of $\omega$ on the boundary $\pd M$. What does this mean? Suppose we have broken up $M$ into infinitesimal regions; for the sake of easy visualization, we assume only that $M$ has been split into two halves down the middle. $d\omega$ measures the net flux out of the boundary of each half of $M$, very roughly. We are thus measuring on the boundaries of the halves of $M$, as follows.
\begin{figure}[H]
\centering
\includegraphics[scale=0.3]{stokes2.png}
\end{figure}
What is happening in the middle? The two halves of $M$ share this part of the boundary, and we are measuring the same thing in opposite orientations and adding, so the middle part does not contribute to the integral $\int_M d\omega$. As the picture suggests, this leaves what is on the boundary, just $\int_{\pd M}\omega$. The real picture looks something like
\begin{figure}[H]
\centering
\includegraphics[scale=0.3]{stokes3.png}
\end{figure}
where the red is the only part that contributes to the integral, because the oriented volumes being measured inside by $d\omega$ in green all cancel out.
