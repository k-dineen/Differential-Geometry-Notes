...
Consider the inclusion map $i : F(N) \to M$. It is obviously a topological embedding. Given $p \in F(N)$, choose a coordinate chart $(F(U), \phi \circ F^{-1})$ for $F(N)$ at $p$ and $(V, \psi)$ for $M$ at $i(p)=p$. If $q = (\phi \circ F^{-1})(p)$, then
\[
\mathrm{rk}(i_{*,p}) = \mathrm{rk}(D( \psi \circ i \circ (\phi \circ F^{-1})^{-1} )(q)) = \mathrm{rk}( D(\psi \circ F \circ \phi^{-1} )(q)) = \mathrm{rk}(F_{*,q}),
\]
which implies that $i_{*,p}$ is injective, so that $i$ is an immersion. Therefore $F(N)$ is a submanifold of $M$. 
